...
const pageContent = data ? data.slice((page - 1) * 8, page * 8) : [];
  ...
  const all = async () => {
    try {
      const res = await axios.get("http://localhost:5000/aorder")
      setData(res.data);
    } catch (error) {
      console.log(error)
    }
                       ...
                       {Array.isArray(pageContent)?pageContent?.map((item,key) => (
                         <tr>
                           ...
                           <td>
                             ...
                             <button className="btn btn-success"> {item.orderstatus}</button>
                             </div>
                           </td>
                         </tr>
                       )):<p>no data</p>}
